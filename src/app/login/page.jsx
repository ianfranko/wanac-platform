  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      try {
        ...
        console.log("Login successful!", { userType, ...form });
        
        // Redirect based on user type
        const dashboardPath = userType === "client" 
          ? "/client" 
          : userType === "coach" 
            ? "/coach" 
            : "/admin";
      ...
      <div className="bg-white shadow-md rounded-lg p-8 max-w-md w-full">
        ...
        <div className="mt-6 text-center space-y-3">
          ...
          <div className="flex space-x-4">
            <button className="flex-1 flex items-center justify-center px-4 py-2 border border-gray-300 rounded-md text-brand-blue hover:bg-gray-50 transition">
        ...
        <p className="text-xs text-center text-gray-500 mt-4">
          Having trouble logging in? <Link href="/contact" className="text-brand-orange hover:underline">Contact support</Link>
        </p>
      </div>
    </div>
  );
