...
export default function IndividualEvaluation({ individualEvaluations, userRole = 'client' }) {
  ...
  const filteredEvaluations = individualEvaluations.filter(evaluation => {
    if (userRole === 'client') {
      // Show only the current user's evaluation — match by user_id from localStorage.
      // The transformer in useEvaluationData uses the same slugify logic to build participantId.
      const rawId = (typeof localStorage !== 'undefined' && localStorage.getItem('user_id')) || '';
      const currentUserId = rawId.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
      // Show entry if IDs match, or if there is only one evaluation (single-user session)
      return evaluation.participantId === currentUserId || individualEvaluations.length === 1;
    }
