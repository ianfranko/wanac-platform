...
<Box>
  ...
  {editExperienceData.agenda.map((step, idx) => (
    ...
    >
      <TextField
        label="Step Title"
        value={step.title ?? ""}   // ✅ ensure string
        onChange={(e) => {
        ...
        onBlur={async (e) => {
          if (!step.id && step.title && step.duration) {
            // create new step in API
            const newStep = await handleAddAgendaStep({
              title: step.title,
            ...
            handleUpdateAgendaStep(step.id, { title: e.target.value });
          }
        }}
        required
        sx={{ flex: 2 }}
      ...
      <TextField
        label="Duration"
        value={step.duration ?? ""}   // ✅ ensure string
        onChange={(e) => {
          const agenda = [...editExperienceData.agenda];
        ...
        onBlur={async (e) => {
          if (!step.id && step.title && step.duration) {
            ...
            handleUpdateAgendaStep(step.id, { duration: e.target.value });
          }
        }}
        sx={{ flex: 1 }}
      />
      ...
      <Button
        onClick={() =>
          step.id
            ? handleDeleteAgendaStep(step.id, idx)
            : setEditExperienceData({
                ...editExperienceData,
