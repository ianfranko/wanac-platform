...
// Reusable ExperienceDetails component
export default function ExperienceDetailsPage({ experienceId: propExperienceId, fireteamId: propFireteamId, isCoach = false }) {
  const params = useParams();
  const router = useRouter();
  // Use either prop or URL param
  const experienceId = propExperienceId || params.experienceId;
  ...
  useEffect(() => {
    console.log("ExperienceDetailsPage: experienceId", experienceId);
    console.log("ExperienceDetailsPage: fireteamId", fireteamId);
  ...
  try {
    ...
    console.log("API response from getExperiences:", response);

    let exp = null;

    // Case 1: API returns an array of experiences
    if (Array.isArray(response.fireTeamExperiences)) {
      exp = response.fireTeamExperiences.find(e => Number(e.id) === Number(experienceId));
    }
    // Case 2: API returns a single experience object
    else if (response.id && Number(response.id) === Number(experienceId)) {
      exp = response;
    }

    console.log("Matched experience:", exp);
    setExperience(exp);
