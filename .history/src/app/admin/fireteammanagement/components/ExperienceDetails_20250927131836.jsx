...
export default function ExperienceDetailsPage({ experienceId: propExperienceId, fireteamId: propFireteamId, isCoach = false }) {
  ...
  const fetchExperience = async () => {
    ...
    try {
      ...
      // Always use fireTeamExperiences array from response
      const experiences = response.fireTeamExperiences || [];
      // Ensure both IDs are numbers for comparison
      const exp = experiences.find(e => Number(e.id) === Number(experienceId));
  ...
  if (loading) {
    return <Box sx={{ p: 4 }}><Typography>Loading experience...</Typography></Box>;
  }
  if (!experience) {
    return <Box sx={{ p: 4 }}><Typography color="error">Experience not found</Typography></Box>;
  }
